...
def local():
    ...
    np.save('vgg_19_mirflickr_features_neb.npy', vgg_features_neb)
...
def autotrain():
    ...
    texts = tf.placeholder(tf.float32, [None, 1386])
    ...
    re_text_image = model.image_decoder_text(image_embedding, 1386)
    ...
    disc_fake_image = model.disc(re_image_text_drop, texts, reuse=False)
    disc_fake_text = model.disc(images, re_text_image_drop, reuse=True)
    ...
    gen_train_op = tf.train.AdamOptimizer(learning_rate=1e-4, beta1=0.5, beta2=0.9).minimize(
        consistency_loss + gen_cost + 0 * var_loss, var_list=gen_params)
    ...
    with tf.Session(config=config) as session:
        ...
        for i in range(14001):
            ...
            session.run(gen_train_op, feed_dict={images: train_images, texts: train_texts})
            if (i % 2000 == 0 and i >= 6000):
                new_map = test()
                if new_map>old_map:
                    old_map = new_map
                    saver.save(session, './referee_advKNN')
            if (i % 100 == 0):
                _closs, _gloss, _var, _t, _i = session.run([consistency_loss, gen_cost, var, loss_text, loss_image],
                                                           feed_dict={images: train_images, texts: train_texts})
                print("consistency_loss:%f, disc_loss:%f, var:%f" % (_closs, _gloss, _var))
                print('image:%f, text:%f' % (_i, _t))
        session.close()

def binary_train(code_dim):
    import model
    BATCH_SIZE = 128
    ...
    binary_vec = model.binary_layer(embeddings, code_dim)
    loss = 0
    for i in range(BATCH_SIZE):
        loss += tf.reduce_mean(tf.square(dist[i, :] - tf.reduce_mean(binary_vec[i] * binary_vec, 1)))
    loss /= BATCH_SIZE
    lossb = tf.square(1 - tf.reduce_mean(binary_vec * binary_vec))
    vars = tf.trainable_variables()
...
def embeddingB(code_dim):
    ...
    model = model.luo(BATCH_SIZE, 64)
    text_embedding = model.txt_encoder(texts)
    text_embedding = tf.sign(model.binary_layer(text_embedding, code_dim))
    image_embedding = model.image_encoder(images)
    image_embedding = tf.sign(model.binary_layer(image_embedding, code_dim, reuse=True))
    config = tf.ConfigProto()
    config.gpu_options.allow_growth = True
    train_data_list = np.load('train_data_list.npy')
    test_data_list = np.load('test_data_list.npy')
...
def disc_train():
    ...
    vgg_features = np.load('vgg_19_mirflickr_features.npy')
    vgg_features_neb = np.load('vgg_19_mirflickr_features_neb.npy')
    vgg_features = np.tanh((vgg_features) / np.std(vgg_features))
    model = model.luo(BATCH_SIZE, 64)
    images = tf.placeholder(tf.float32, [None, 4096])
    texts = tf.placeholder(tf.float32, [None, 1386])
